...
def register():
   ...
   if '<'  in username:
      flash("Invalid use of metachars in username")
      return redirect(url_for('register'))
   if '>' in username:
      flash("Invalid use of metachars in username")
      return redirect(url_for('register'))
...
def update_profile():
   ...
   if not new_photo:
      ...
      if new_photo.filename[-4:] != ".png" or new_photo.filename[-4:] != ".jpg":
         flash("Invalid format for photo, use .png or .jpg")
         return render_template('profile.html', current_user=current_user)
...
def create_post():
   ...
   try:
      new_content =cgi.escape(new_content)
